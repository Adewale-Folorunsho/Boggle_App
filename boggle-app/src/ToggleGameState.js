  ...
  */
  ...
  async function showLeaderBoard(){
    ...
    try {
       ...
       const newBoards = querySnapshot.docs.map((doc) => {
       console.log(doc.id, " => ", doc.data());
       return doc.data(); //doc.boardSize, doc.solveTime, doc.playerName
      });
        
      setLeaderBoard([...newBoards, ...leaderBoard]);
  ...
  return (
    ...
    <div className="Toggle-game-state">
      ...
      { gameState === GAME_STATE.ADD_LEADERBOARD &&
        ...
        <TextField value={input} id="outlined-basic" label="Enter Your Name" variant="outlined" onKeyPress={(e) => keyPress(e)} onChange={(event) => setInput(event.target.value)} />
      }
